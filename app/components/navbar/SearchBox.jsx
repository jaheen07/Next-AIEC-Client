...
const SearchBox = () => {
  ...
  return (
    <div className="relative top-0 hidden xl:block left-5">
      <div className="relative flex items-center">
        <input
          className="px-5 pl-10 glass text-sm font-normal rounded-lg w-[350px] mx-auto h-[41px] relative border-black/25 border-2 text-black"
          placeholder={
            language === "bn"
        ...
        <>
          ...
          {(CourseData.length > 0 || consultantData.length > 0) && (
            <section className="absolute w-[50vw] mx-auto top-[60px] bg-white border p-3 shadow-lg flex justify-between">
              <div className="w-1/2 p-3 overflow-y-auto border-r-2 h-96 border-black/10">
                {CourseData.length > 0 ? (
                  <div>
                    ...
                    {CourseData?.map((course) => (
                      <Link
                        ...
                        className="flex items-start gap-2 pb-1 my-4 border-b-2"
                        key={course._id}
                      >
                        <img
              ...
              {/* <div className="w-1/2 p-3 overflow-y-auto h-96">
                    ...
                    {consultantData?.map((c) => (
                      <Link
                        to={`/consultant/${c.displayName
                          ...
                          .replace(/\s+/g, "-")}`}
                        onClick={() => setIsOpen(false)}
                        className="flex gap-2 pb-1 my-4 border-b-2"
                        key={c._id}
                      >
                        <img
                          className="w-12 h-12 rounded-full"
