...
const BlogCard = ({ blog }) => {
  ...
  return (
    <div className="flex flex-col justify-between w-full p-4 border border-black rounded-md">
      ...
      <div className="flex items-end justify-between mt-2">
        <div>
          <Link href={`/blogs/${blog.blogName.trim().replace(/\s+/g, '-')}`}>
            <button className="btn-black rd-more" onClick={incrementViewCount}>
              {/* <FaArrowRight /> Read More */} Read More
            </button>
          </Link>
        </div>
