...
export async function generateStaticParams() {
  const course = await fetch('https://server.aiexpertcareer.com/workshops', { cache: "no-store"}).then((res) => res.json())
...
export default async function Page({params})  {
  ...
  const course = await fetch(`https://server.aiexpertcareer.com/single-workshop/${name.replace(/-/g, ' ').trim()}`,{ cache: "no-store"}).then((res)=>res.json());
  ...
  return (
    ...
    //               <ExportedImage src={insImage} className="object-contain w-20 h-20 rounded-full"  alt="instructor Photo" width={70} height={20}/>
    //             </figure>
    //             <div>
    ...
    <div className="w-full mt-20">
      ...
      <section className="max-w-full gap-6 px-4 py-2 mx-auto my-5 md:mt-28 md:my-10 xl:max-w-7xl md:px-10 xl:flex xl:gap-10 ">
        {/* Left Side Contents */}

        
        <section className="space-y-5 md:w-[70%] md:space-y-7">
          <h1 className="text-3xl font-bold md:text-4xl">{title}</h1>
          <h2 className="md:text-xl sm:text-sm">{subtitle}</h2>
          <hr />
          

          {/* Main features for mobile view */}
          <div className="text-md md:w-96 md:hidden">
            ...
            <div className="p-1">
              <h3 className="my-3 text-lg font-bold">
                {" "}
                {language == "bn"
                  ? "এই ওয়ার্কসপ থেকে যা যা শিখবেন"
                  : "Workshop Main Features:"}
              </h3> 
              <div className="grid grid-cols-2 gap-x-2">
                {features?.map((feature, i) => (
                    <p className="flex items-center gap-3 my-2 text-md" key={i}>
                      <CheckCircleOutlined className="text-lg text-red-500"/>
              ...
              {modules?.map((module, i) => (
                <div key={i}>
                  <details className="group [&_summary::-webkit-details-marker]:hidden rounded-xl p-2 bg-white ">
                    ...
                    <p className="px-2 mt-4 leading-relaxed text-gray-700 ">
                      {module.contents.map((c,i) => (
                        <div key={i} className="flex items-center justify-between">
                          <p className="flex items-center gap-3 my-3 font-semibold cursor-pointer text-md md:text-lg">
                            { c.type == "assignment" ? <span><img className="w-5" src={assI} alt="" /></span> : c.type =="quiz" ? <span className="w-5">{quizI}</span> : <span><PlaySquareOutlined/></span>}
                                
                            <span className="w-[90%] md:w-full">{c.title}</span> 
                          </p>

                          <span><LockOutlined className="text-gray-400" /></span>
                        </div>
                      ))}
                    </p>
                  </details>
          ...
          <div>
            <h3 className="my-1 text-xl font-bold md:text-2xl">
              {" "}
              {language == "bn" ? "ওয়ার্কসপের বিবরণ:" : "Workshop Details:"}
            </h3>
            <div className="section bg-slate-100">
              <p dangerouslySetInnerHTML={{ __html: description }}></p>
            </div>
        ...
        {/* right Side Contents */}
        <section className="my-5 md:my-0 md:w-[45%] w-full">
            <div className="hidden md:w-full md:block">
              {coverVideo ? (
                <ReactPlayer   width="100%"
                  url={coverVideo} />  
              ) : (
                <img src={cover} alt="cover" className="w-full mx-auto rounded-t-lg" />
              )}
            </div>
          {/* Features */}
          {/* <div className="hidden text-sm section md:w-96 md:block">
            ...
            <div className="sticky hidden mt-1 top-20 md:block">
              <div className="p-2 mt-1 text-base border rounded-b-xl course-details">
                {/* paid course enrollment */}
                {courseType === "paid" ? (
                  <>
                    {" "}
                  ...
                  //  free course enrollment
                  <>
                    <p className="my-5 text-xl font-semibold">{language == "bn" ? "ওয়ার্কশপ মূল্য:" : "Workshop Fee:"}<span className="float-right mx-6">৳ ০</span></p>
                  {
                    enrolled ? 
                    <div>
                      <p>You&apos;ve registered Already</p>
                      <Link className="w-full my-2 text-white bg-black btn hover:bg-black hover:text-white " to='/dashboard/my-courses'> Go To Dashboard <IoIosArrowForward/></Link>
                    </div> : 
                    <Link
                    state={{ _id, title, workshop }}
                    to="/free-course"
                ...
                <div className="p-2 mt-2 ">
                  ...
                  <div className="grid grid-cols-2 gap-x-2" >
                    {features?.map((feature, i) => (
                        <p className="flex gap-3 my-1 text-lgitems-center"key={i}>
                          <CheckCircleOutlined className="flex text-lg text-red-500" />
                          <span className="flex w-[90%]">{feature}</span>
                        </p>
                    ))}   
                  </div>
                </div>
              ...
              {courseType === "paid" && (
                <>
                  {/* <CountDown startDate={startDate} endDate={endDate} courseDate={courseDate} /> */}
                  <div className="flex justify-between mt-2 section">
                    <div className="">
                      <h3 className="my-3 font-semibold text-md">
                        {" "}
                        {language == "bn" ? "ভর্তি শেষ:" : "Admission will end on:"}
      ...
      <div className="md:hidden">
        {/* {isVisible && (
          <div className="fixed bottom-0 left-0 w-full max-h-[80vh] bg-white border shadow-lg z-10 overflow-y-auto">
            {enrolled?
            "":
            <p className="px-6 py-1 pl-3 text-xl font-semibold text-left md:text-2xl">
              
              {courseFee > 0 ? <>{discount ? discount != 0 ? <><span className="mx-2 text-gray-500 line-through text-md">৳ {courseFee}
            
            </span> <span className="text-xl">৳{discountAmount}</span></>:"": <>{spdiscount?<span className="mx-2 text-gray-500 line-through text-md">৳ {courseFee}</span>  :""}<span className="text-xl">৳{courseFee - spdiscount}</span></>}</> : "Free"}
            
            {courseType === "paid"?
            <span className="float-right"><Link onClick={()=> document.getElementById("my_modal_5").showModal()} state={{ workshop, discountAmount, courseFee }} className="text-sm text-gray-600 underline">{language == "bn" ? "প্রোমো কোড" : "Promo Code"}</Link></span>:""}
            </p>
            }
            <div className="">
              { enrolled ?
                ( <div className="grid grid-cols-2 px-2">
                    <span className="grid my-auto text-sm font-semibold">You&apos;ve Registered Already</span>
                    <Link className="grid my-3 text-sm text-white bg-black btn hover:bg-black hover:text-white" to='/dashboard/my-courses'>Go To Dashboard</Link>
                   </div>
                ): 
                (courseType === "paid" ? 
                  <Link
                  ...
                  (courseType === "free"? 
                    <Link to="/free-course" state={{ workshop, discountAmount, courseFee, _id, title  }} className="w-11/12 mb-2 bg-white btn-view-red">
                      Register Now <IoIosArrowForward/>
                    </Link>:""
                  )
                ) 
